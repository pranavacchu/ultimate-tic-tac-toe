...
const GameBoard = () => {
  ...
  const [mainBoardState, setMainBoardState] = useState<Array<string>>(
    Array(9).fill("")
  );
  ...
  const updateGameStateFromResponse = (
    ...
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j < 9; j++) {
        ...
        newGameState[Math.floor(i / 3) * 3 + Math.floor(j / 3)][
          (i % 3) * 3 + (j % 3)
        ] = value === 1 ? "X" : value === -1 ? "O" : "";
      }
    }
    ...
    const newMainBoard = data.meta_board
      .flat()
      .map((value: number) => (value === 1 ? "X" : value === -1 ? "O" : ""));
    ...
    // Check for winner - only check if this is the bot's move or if player won
    if (!isPlayerMove || data.winner === 1) {
      if (data.winner !== null) {
  ...
  const handleCellClick = async (boardIndex: number, cellIndex: number) => {
    ...
    try {
      ...
      const playerMove = {
        ...
        is_player_move: true, // Add flag to indicate this is just the player's move
      };

      const playerResponse = await fetch("http://localhost:8000/make-move", {
        method: "POST",
        headers: {
      ...
      updateGameStateFromResponse(playerData, true);

      // If game isn't over after player's move, make bot's move after delay
      if (playerData.winner === null) {
        // Use consistent 1 second delay for all difficulties
        await new Promise((resolve) => setTimeout(resolve, 1000));

        // Now make the bot's move
        const botMove = {
          ...playerMove,
          is_player_move: false, // Indicate this is the bot's move
        };

        const botResponse = await fetch("http://localhost:8000/make-move", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(botMove),
        });
        ...
        updateGameStateFromResponse(botData);
      }
    } catch (error) {
  ...
  const renderMainBoard = () => {
    return (
      <div className="bg-black/30 p-4 rounded-xl backdrop-blur-sm">
        <h3 className="text-white/80 text-lg mb-3 font-medium text-center">
          Game Progress
        </h3>
    ...
    return (
      <button
        className={`w-full h-full flex items-center justify-center text-3xl font-bold transition-all duration-300
          ...
          ${!value && isPlayable ? "hover:bg-white/30 hover:scale-110" : ""}
          ${!isPlayable ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}
          font-game tracking-wide rounded-full relative
        `}
        ...
        {value ? (
          ...
          <div
            className={`w-2 h-2 rounded-full ${
              isPlayable ? "bg-white/20 hover:bg-white/50" : "bg-white/10"
            }`}
          />
        )}
      </button>
  ...
  const renderBoard = (boardIndex: number) => {
    ...
    return (
      <div
        className={`relative backdrop-blur-sm rounded-lg p-3 transition-all duration-300
        ${
          isPlayable
            ? "ring-2 ring-white/20 hover:ring-tictac-purple/50 shadow-lg shadow-tictac-purple/20"
            : "opacity-75"
        }
          ...
          <div className="win-message-trophy">
            <Trophy
              className={`w-20 h-20 mx-auto mb-6 ${
                winner === "X" ? "text-tictac-blue-light" : "text-tictac-purple"
              }`}
            />
          </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-start max-w-5xl mx-auto">
          ...
          <div className="flex-1 mr-6">
            <div
              className="grid grid-cols-3 gap-3 md:gap-4 p-4 rounded-xl 
          ...
          <div className="w-56 sticky top-8">{renderMainBoard()}</div>
        </div>
      </div>
    </div>
  );
